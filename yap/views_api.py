	...
	def post(self,request):
		...
		if check[1]:
			...
			if kwargs.get('facebook_shared_flag') == True and user.settings.facebook_connection_flag == True:
				...
				f2 = facebook.share_yap_story_on_facebook(user=user,yap=yap,facebook_access_token=facebook_access_token)
...
class ListOfUsersWhoLikedObject(APIView):
		...
		if check[1]:
			if obj_type == "yap":
				try:
					...
					return Response({'valid':False,'message':'This yap does not exist.'})
				if 'after' in request:
					like_objects_of_users_that_liked_this_object = Like.objects.filter(is_active=True,yap=obj,date_created__lt=dateutil.parser.parse(request.get('after')))
				else:
					like_objects_of_users_that_liked_this_object = Like.objects.filter(is_active=True,yap=obj)
			elif obj_type == "reyap":
				try:
					...
					return Response({'valid':False,'message':'This reyap does not exist.'})
				if 'after' in request:
					like_objects_of_users_that_liked_this_object = Like.objects.filter(is_active=True,yap=obj.yap,date_created__lt=dateutil.parser.parse(request.get('after')))
				else:
					like_objects_of_users_that_liked_this_object = Like.objects.filter(is_active=True,yap=obj.yap)
			users_that_liked_this_object = sorted(set(like_objects_of_users_that_liked_this_object),reverse=True,key=attrgetter('date_created'))[:request.get('amount')]
			serialized = YapDetailsListOfLikersAndReyappers(users_that_liked_this_object,many=True,data=self.request.DATA,context={'user':user})
			return Response(serialized.data)
		else:
			return Response(check[0])

class ListOfUsersWhoReyappedObject(APIView):
